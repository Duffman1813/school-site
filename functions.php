<?php
/**
 * school-site-theme functions and definitions
 *
...
function school_setup() {
	/*
		...
		* If you're building a theme based on school-ste-theme, use a find and replace
		* to change 'school-ste-theme' to the name of your theme in all the template files.
		*/
	...
	add_image_size('student-pic',300, 200, true);
	add_image_size('students-pic',200, 300);
...
function school_theme_color(){
	...
	add_action( 'wp_head', 'school_theme_color' );
...
function school_excerpt_more( $more ){
	...
	$more = '... <a class="read-more" href="'. esc_url( get_permalink() ) . '">'. __( 'Read more...', 'school' ).'</a>';
...
function school_archive_title_prefix( $prefix ){
    if ( get_post_type() === 'school-students' ) {
        return false;
    } else {
        return $prefix;
    }
